...
def get_user(user_id):
	return frappe.db.get_value(
		"Discourse ID Map", {"discourse_table": "users", "discourse_id": user_id}, "reference_name"
	)
...
def get_project(category_id):
	return frappe.db.get_value(
		"Discourse ID Map",
		{"reference_doctype": "GP Project", "discourse_table": "categories", "discourse_id": category_id},
		"reference_name",
	)
...
def get_reactions(post_id):
	likes = run_query(f"""
		...
		where post_action_type_id = 2 and user_id > 0 and post_id = {post_id};
	""")
	reactions = []
	for d in likes:
		reactions.append({"user": get_user(d.user_id), "emoji": "💖"})
	return reactions
...
def clear_data(doctypes=None):
	...
	for dt in doctypes:
		for row in frappe.db.get_all(
			"Discourse ID Map", {"reference_doctype": dt}, ["reference_name", "name"]
		):
