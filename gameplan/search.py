...
class Search:
	...
	def add_document(self, id, doc):
		...
		super(RedisWrapper, self.redis).hset(key, mapping=mapping)

	def remove_document(self, doc):
		doc = frappe._dict(doc)
		key = self.redis.make_key(f"{self.prefix}:{doc.id}").decode()
		self.redis.ft(self.index_name).delete_document(key)

	def search(self, query, start=0, page_length=50, sort_by=None, highlight=False):
