  ...
  it("comment actions", () => {
    ...
    cy.get("@comment")
      ...
      .then((comment) => {
        ...
        // add a reaction
        cy.get(
          `div[data-id=${comment.name}] button[aria-label="Add a reaction"]`,
        ).click();
        ...
        cy.get(`div[data-id=${comment.name}]`)
          .contains("This is an edited comment")
          .should("exist");
        cy.get(
          `div[data-id=${comment.name}] .mention[data-id="john@example.com"]`,
        ).should("exist");
